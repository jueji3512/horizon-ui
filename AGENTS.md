...
- 当前工作分支为本地 `dev`；旧 `codex/docs-and-icon-rework` 本地分支已删除，远端当前仍只有 `origin/master`。截至 2026-06-20，已继续完成 Progress 视觉修正和 `size` API 收口：circle active 已对齐为和 line 同源的渐变 stroke 扫光，并校正为从进度起点扫向当前进度端点；`size` 为 `sm|md|lg|ProgressSizeConfig`，对象字段 `thickness`、`labelSize`、`diameter` 未指定时回退 `md`，不再支持数字直传。下次如无新的 Progress 反馈，可推进 Notification。新对话以 `git status --short` 与 `git log -1 --oneline` 为准。
...
- 2026-06-15 Progress v1 已落地；2026-06-20 继续完成一轮视觉打磨和尺寸 API 收口：公开确定进度组件支持 `variant="line|circle"`、`percent` clamp、`theme="brand|success|warning|error"`、line `sm/md/lg` 高度 4/6/8px、circle `sm/md/lg` 直径 72/120/160px、`ProgressSizeConfig` 对象式自定义、`color` 覆盖进度填充色、label/icon 互斥和 ARIA；对象字段为 `thickness`（line track height / circle stroke width）、`labelSize`、`diameter`，未指定字段回退 `md`，状态图标跟随 `labelSize`，line 为 `1em`、circle 为 `2.4em`；不支持未知进度、steps、buffer、success percent 或 `size` 数字直传。
...
- 2026-06-15 Progress v1 已落地，2026-06-20 完成视觉修正与尺寸 API 收口：只表达确定百分比，线性和环形形态共享 percent clamp、theme / color、label / icon 互斥和 ARIA；line 与 circle 均仅在 brand 且未满时支持 active 流动动画，circle active 使用与 line 同源的渐变 stroke 扫光；`size` 支持 `sm/md/lg` 预设和 `ProgressSizeConfig`，不再支持数字直传；环形状态示例已合并回 `docs/examples/progress/example-02.vue`，不再单独保留“环形状态”章节；`npm run check:progress` 已纳入 `npm run check`。
