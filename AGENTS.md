...
npm run check:time-panel # 校验 TimePanel 时间列面板契约
...
npm run check        # format:check + check:component-demo + check:icons + check:scroll-area + check:time-panel + check:popper + check:popover + check:menu + check:select + check:input-number + check:form + check:dialog + check:drawer + check:message + check:progress + check:notification + check:dropdown-menu + lint + typecheck + build
```
...
- 截至 2026-06-23，已继续完成 Progress 视觉修正、`size` API 收口和状态 API 调整、Notification v1、Drawer v1 与 TimePanel v1：Drawer 是单一公开组件，使用 `open` / `v-model:open` / `open-change`，不提供 trigger/content/close primitive，内部复用 Dialog modal layer 的 top-layer、scroll lock、focus trap、关闭后焦点恢复和 Dialog 内 Teleport 子浮层 LIFO Esc 能力；支持 `placement="right|left|top|bottom"`、默认左右 400px / 上下 320px 尺寸、通过 `class` / `style` / `panelClass` 控制面板尺寸、title/description/aria label、footer slot close helper、Esc / overlay click / close icon。TimePanel 是公开底层时间列面板，不处理日期、输入框、浮层触发、自由输入解析、确认操作或表单接入；支持 `HH:mm`、秒、毫秒、12 小时 AM/PM、`steps`、`disabledTime`、即时选择、Expose 方法和键盘列选择，Now / Clear / OK / Cancel 等操作留给后续上层时间输入组件。
- 最近一次完整验证：2026-06-23 `npm run check` 通过，包含 format、check:component-demo、check:icons、check:scroll-area、check:time-panel、check:popper、check:popover、check:menu、check:select、check:input-number、check:form、check:dialog、check:drawer、check:message、check:progress、check:notification、check:dropdown-menu、lint、typecheck 和 VitePress build；build 仅保留 VitePress chunk size warning。
...
- 2026-06-23 TimePanel v1 已落地：公开底层 `TimePanel` 只负责时间列选择，不做完整输入框、浮层或确认操作组合；`modelValue` 按 `format` 输出，支持 `HH:mm`、`HH:mm:ss`、`HH:mm:ss:SSS`、`hh:mm A`、`hh:mm:ss A`、`hh:mm:ss:SSS A`，由格式决定秒 / 毫秒 / AM-PM 列；支持 `steps` 四元组、`disabledTime`、即时选择、ScrollArea 滚动列、方向键 / Home / End / Enter 键盘交互，并通过 `TimePanelExpose` 暴露 `focus` / `scrollToActive` / `getValue` / `getParts` / `setValue` / `setNow` / `clear`；`npm run check:time-panel` 已纳入 `npm run check`。
...
- 已实现并纳入文档：Button、Icon、Link、Checkbox、Radio、Text、Title、Callout、Divider、Badge、Tooltip、Popover、Dialog、Drawer、Message、Progress、Notification、Menu、DropdownMenu、Switch、Input、InputNumber、Select、Form、FormItem、Tag、Space、Field、Popper、ScrollArea、TimePanel。
- Badge 已加入 sidebar。
...
- 2026-06-12 已记录文档站后续方向：当前 `data-horizon-teleport-layer` + `postcssIsolateStyles` 的 Teleport 层隔离是沿用默认 VitePress theme 期间的阶段性方案；后续应参考 Element Plus 自定义 VitePress shell / demo 渲染层，完成后移除该临时隔离标记、白名单和对应检查。
...
- 2026-06-06 已先不继续使用 Figma，改为在项目内补充 48 个常用本地图标；当前 `src/components/Icon/icons/` 共 101 个 SVG，新增图标覆盖导航、数据、表单、反馈、权限、文件、操作和系统场景；2026-06-20 为 Progress line 状态补充 3 个填充圆形状态图标。图标命名以 Horizon 对外简洁名为准，不直接照搬来源库后缀（例如 `grid`、`building`），VitePress Icon 页保持轻量搜索网格并将相关图标相邻展示，通过 `npm run check:icons` 必备列表、root `<svg>` 标签碎片、BOM 和子 `<rect>` 几何检查守护。
...
- 2026-06-10 Form / FormItem 首版已落地：默认企业表单视觉为左侧 label、label 文案右对齐、`labelWidth=120`；help 通过 label 处问号 tip 表达，控件下方消息行只放校验 / 状态文案并默认预留高度；校验逻辑集中在 `src/components/Form/validator.ts`，`npm run check:form` 已纳入 `npm run check`。
- 2026-06-11 Dialog v1 已落地：单一公开组件，公开状态统一为 `open` / `v-model:open` / `open-change`，不提供 trigger primitive；内部 modal layer 处理 top-layer、10000+ modal z-index、Esc、overlay click、focus trap、scroll lock、关闭后焦点恢复和 Dialog 内 Popper / Select 等 Teleport 子浮层协调；`npm run check:dialog` 已纳入 `npm run check`。
- 2026-06-14 Message v1 已落地：命令式全局 toast 服务，Signal Rail 紧凑视觉，`message.loading()` 默认不自动关闭，同 key 更新替换已有消息，`message.config()` 支持 duration / closable / max / top / zIndex；`npm run check:message` 已纳入 `npm run check`。
- 2026-06-15 Progress v1 已落地，2026-06-20 完成视觉修正、尺寸 API 收口和状态 API 调整：只表达确定百分比，线性和环形形态共享 percent clamp、status / color、label / icon 互斥和 ARIA；line 与 circle 均仅在默认态且未满时支持 active 流动动画，circle active 使用与 line 同源的渐变 stroke 扫光；`size` 支持 `sm/md/lg` 预设和 `ProgressSizeConfig`，不再支持数字直传；结果状态使用 `status`，默认 brand 不是 status 枚举；动态更新示例已补充；`npm run check:progress` 已纳入 `npm run check`。
- 2026-06-20 Notification v1 已落地：结构化全局通知服务，不公开用户态 `<Notification />` 组件；公开 `notification.info/success/warning/error/loading/close/closeAll/config`，支持四角 placement、同 key 更新、普通通知默认 4500ms、loading 默认 `duration: 0`、固定 360px 桌面宽度、统一向上淡出离场、`progress` 线性任务进度、`progress.status` 任务结果、可选 action 和全局 config；`npm run check:notification` 已纳入 `npm run check`。
...
- `data-horizon-teleport-layer` 只用于当前文档站默认 theme reset 隔离，不是长期组件 API；未来自定义文档 shell 成熟后应删除它及相关检查。
