...
- [ ] 继续整理组件级固有尺寸规范：Switch、Badge、Tooltip / PopperArrow、Checkbox / Radio 已定稿为组件内部几何规格；FieldAction、InputNumber 输入段宽度等仍需逐项确认保留内部常量还是沉淀组件级 token。
...
- [ ] 组件级固有尺寸尚未统一规范，例如 FieldAction、InputNumber 输入段宽度等；Switch 轨道与滑块、Badge 点/计数尺寸、Tooltip surface、PopperArrow、Checkbox / Radio 控件几何已先定为组件内部几何规格，不新增通用 token。
