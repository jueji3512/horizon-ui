  ...
  <Teleport :to="resolvedTeleportTarget" :disabled="!mounted">
    <Transition name="h-dialog-fade">
      <div
        ...
        class="h-dialog-overlay fixed inset-0 flex items-center justify-center bg-black/35 px-4 py-6"
        :style="overlayStyle"
      ...
      >
        ...
        >
          ...
          <footer v-if="$slots.footer" class="flex justify-end gap-2 px-6 pb-6">
...
import type { DialogFooterSlotProps, DialogRole, DialogSize } from './types'
import {
  focusFirstElement,
  isTopLayer,
  lockBodyScroll,
...
const layerId = Symbol('dialogLayer')
const titleId = useId()
...
const panelClasses = computed(() =>
  cn(
    'h-dialog-panel max-h-[calc(100vh-48px)] w-full overflow-hidden rounded-[var(--round-default)] bg-[var(--bg-color-container)] text-[var(--text-color-primary)] shadow-2xl outline-none',
...
const bodyClasses = computed(() => cn('px-6', slots.footer ? 'pb-5' : 'pb-6'))

const footerSlotProps = computed<DialogFooterSlotProps>(() => ({
  close,
}))
...
.h-dialog-fade-enter-active .h-dialog-panel,
.h-dialog-fade-leave-active .h-dialog-panel {
  transition:
    opacity var(--duration-normal) ease,
    transform var(--duration-normal) ease;
}

.h-dialog-fade-enter-from .h-dialog-panel,
.h-dialog-fade-leave-to .h-dialog-panel {
  opacity: 0;
