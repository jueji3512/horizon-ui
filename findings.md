...
- 公开 API 为 `variant="line|circle"`、`percent`、`status="success|warning|error"`、`size="sm|md|lg|ProgressSizeConfig"`、`active`、`color`、`showLabel`、`label`、`ariaLabel`；不再支持 `size` 数字直传或旧 Progress `theme` API，默认 brand 进度色不作为 status 枚举暴露。
