...
npm run check        # format:check + check:component-demo + check:icons + check:scroll-area + check:popper + check:popover + check:menu + check:select + check:input-number + check:form + check:dialog + check:message + check:progress + check:dropdown-menu + lint + typecheck + build
```
...
- 当前工作分支为本地 `dev`；旧 `codex/docs-and-icon-rework` 本地分支已删除，远端当前仍只有 `origin/master`。截至 2026-06-20，已继续完成一轮 Progress 视觉修正：环形中心标签按直径递进为 `font-body-sm` / `font-body-md` / `font-body-lg`，circle active 已对齐为和 line 同源的渐变 stroke 扫光，并校正为从进度起点扫向当前进度端点；下次新对话先继续讨论 Progress 的尺寸 / `size` props，再决定是否推进 Notification。新对话以 `git status --short` 与 `git log -1 --oneline` 为准。
- 最近一次完整验证：2026-06-20 `npm run check` 通过，包含 format、check:component-demo、check:icons、check:scroll-area、check:popper、check:popover、check:menu、check:select、check:input-number、check:form、check:dialog、check:message、check:progress、check:dropdown-menu、lint、typecheck 和 VitePress build；build 仅保留 VitePress chunk size warning。
...
- 2026-06-15 Progress v1 已落地；2026-06-20 继续完成一轮视觉打磨：公开确定进度组件支持 `variant="line|circle"`、`percent` clamp、`theme="brand|success|warning|error"`、line `size="sm|md|lg"` 高度 4/6/8px、circle `size="sm|md|lg"` 直径 72/120/160px 与数字直径、`color` 覆盖进度填充色、label/icon 互斥和 ARIA；track 使用 `--bg-color-component`，流动动画只在 brand line / circle 上出现，circle active 使用与 line 同源的渐变 stroke 扫光，line 状态图标使用填充圆形图标，circle 状态图标使用无圆底图标，环形中心标签随直径使用 `font-body-sm/md/lg`；不支持未知进度、steps、buffer 或 success percent。
- Progress 后续尺寸 API 待下次确认：当前 `size="sm|md|lg"|number` 是首版；用户倾向预设 `sm/md/lg` + 对象式自定义，未设定字段回退 `md`，对象字段候选包括条宽 / stroke 宽度、`labelSize` px、图标字号跟随 `labelSize` 的 `2.4em` 和 circle 直径。
...
- 已实现并纳入文档：Button、Icon、Link、Checkbox、Radio、Text、Title、Callout、Divider、Badge、Tooltip、Popover、Dialog、Message、Progress、Menu、DropdownMenu、Switch、Input、InputNumber、Select、Form、FormItem、Tag、Space、Field、Popper、ScrollArea。
- Badge 已加入 sidebar。
...
- 2026-06-15 Progress v1 已落地，2026-06-20 完成一轮视觉修正：只表达确定百分比，线性和环形形态共享 percent clamp、theme / color、label / icon 互斥和 ARIA；line 与 circle 均仅在 brand 且未满时支持 active 流动动画，circle active 使用与 line 同源的渐变 stroke 扫光，circle 支持 72/120/160px 与数字直径，环形中心标签随直径使用 `font-body-sm/md/lg`；环形状态示例已合并回 `docs/examples/progress/example-02.vue`，不再单独保留“环形状态”章节；`npm run check:progress` 已纳入 `npm run check`。
- 下次继续 Progress 时先处理尺寸 / `size` props：评估预设规格和对象式自定义的类型、默认回退、文档示例与契约检查；用户明确说该事项下次新对话再做。
