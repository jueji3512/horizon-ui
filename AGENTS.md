...
- 当前工作分支为本地 `dev`；旧 `codex/docs-and-icon-rework` 本地分支已删除，远端当前仍只有 `origin/master`。截至 2026-06-20，已继续完成 Progress 视觉修正、`size` API 收口和状态 API 调整：circle active 已对齐为和 line 同源的渐变 stroke 扫光；`size` 为 `sm|md|lg|ProgressSizeConfig`，对象字段 `thickness`、`labelSize`、`diameter` 未指定时回退 `md`，不再支持数字直传；公开结果态改为 `status="success|warning|error"`，默认 brand 进度色不作为 status 暴露，旧 Progress `theme` API 不再使用。下次如无新的 Progress 反馈，可推进 Notification。新对话以 `git status --short` 与 `git log -1 --oneline` 为准。
